...
const switching = ref(false); // đang phủ trắng + nền trượt
const revealing = ref(false); // giai đoạn form mới xuất hiện
let timers = [];
...
function open(defaultMode = "login") {
  ...
  switching.value = false; // đảm bảo không ở trạng thái chuyển
  revealing.value = true; // CHÌA KHÓA: bật input ngay khi mở
  show.value = true;
}
function close() {
  show.value = false;
...
function switchMode(next) {
  ...
  // Sau khi nền trượt xong + delay → reveal form mới
  timers.push(
    setTimeout(() => {
  ...
  // Kết thúc toàn bộ chu trình → tắt switching
  timers.push(
    setTimeout(() => {
      switching.value = false;
    }, TOTAL_SWITCH)
...
const handleLogin = async () => {
  ...
  try {
    // const res = await axios.post("http://localhost:8082/api/auth/login", {
      const res = await axios.post("https://daugiabe-production.up.railway.app/api/auth/login", {
      email: email.value,
...
const handleRegister = async () => {
  ...
  try {
    const res = await axios.post(
      "https://daugiabe-production.up.railway.app/api/users/create",
      {
        ho: registerHo.value,
...
function handleForgotPassword(e) {
  e?.preventDefault?.();
  showToast("Tính năng Quên mật khẩu sẽ sớm có.", "success");
}
